...
if __name__ == '__main__':
    ...
    client_bid_2 = [[0,0], [0,5], [0,10], [0,100], [0,1000], [5,0], [10,0], [100,0], [1000,0]]
    ...
    tau_alpha = [0.2, 0.5]
    outdir_root = 'exp_consistent_repeat_tau_alpha'
    our_dir_name_prefix = 'exp_ls_epoch_alpha_tune_'
    yaml_file_name_prefix = 'alpha_tune_fedex_for_cifar10_'
    yaml_root = 'scripts/marketplace/example_scripts/ls_run_scripts_repeated_exp_tau_alpha/two_clients'
    ...
    with open(original_yaml_file_2, 'r') as f:
        ...
        for client_bid in client_bid_2:
            for seed_val in seed:
                original_yaml['marketplace']['alpha_tune']['client_bid'] = client_bid
                ...
                for entropy_determined_val in entropy_determined:
                    ...
                    client_bid_text = ':'.join('{}'.format(item) for item in client_bid)
                    if entropy_determined_val:
                        file_post = '{}_client_{}_seed_{}_entropy'.format(len(client_bid), client_bid_text, seed_val)
                        original_yaml['outdir'] = os.path.join(outdir_root, our_dir_name_prefix + file_post)
                        ...
                        for tau_alpha_val in tau_alpha:
                            file_post = '{}_client_{}_seed_{}_tmp_alpha_{}'.format(len(client_bid), client_bid_text, seed_val, tau_alpha_val)
                            original_yaml['marketplace']['alpha_tune']['tau_alpha'] = tau_alpha_val
                            original_yaml['outdir'] = os.path.join(outdir_root, our_dir_name_prefix + file_post)
                            yaml_file_name = yaml_file_name_prefix + file_post + '.yaml'
                            yaml_sav_pth = os.path.join(yaml_root, yaml_file_name)
                            yaml_file_pth_list_two_clients.append(yaml_sav_pth)
        ...
        yaml_root = 'scripts/marketplace/example_scripts/ls_run_scripts_repeated_exp_tau_alpha/three_clients'
                ...
                for seed_val in seed:
                    ...
                    for entropy_determined_val in entropy_determined:
                        ...
                        if entropy_determined_val:
                            ...
                            yaml_file_name = yaml_file_name_prefix + file_post + '.yaml'
                            yaml_sav_pth = os.path.join(yaml_root, yaml_file_name)
                            yaml_file_pth_list_three_clients.append(yaml_sav_pth)
                            optional_run_set.append(yaml_sav_pth)
                            with open(yaml_sav_pth, 'w') as yaml_f:
                                yaml.dump(original_yaml, yaml_f)
                        else:
    ...
    total_run = (len(yaml_file_pth_list_three_clients) + len(yaml_file_pth_list_two_clients))/(len(available_gpu)*per_gpu_running)
    ...
    sh_pth = 'scripts/marketplace/rep_sh_rerun'
    check_dir(sh_pth)



    while len(optional_run_set)>0:
        ...
        for gpu_id in available_gpu:
            for i in range(per_gpu_running):
                ...
                count += 1
                if count == limit:
                    bash_pth = os.path.join(sh_pth, 'run_{}.sh'.format(bashfile_id))

            ...
            for seed_val in seed:
                for train_weight_val in train_weight_3:
                    ...
                    for entropy_determined_val in entropy_determined:
                        ...
                        train_weight_txt = ':'.join(
                            '{}'.format(item) for item in train_weight_val)
                        if entropy_determined_val:
                            ...
                            for tau_alpha_val in tau_alpha:
                                file_post = '{}_client_{}_seed_{}_train_weight_{}_tmp_alpha_{}'.format(
                                    len(client_bid), client_bid_text, seed_val,
                                    train_weight_txt, tau_alpha_val)
                                original_yaml['marketplace']['alpha_tune'][
...
if __name__ == '__main__':
    ...
    train_weight_2 = [[0.9, 0.1], [0.8, 0.2], [0.3, 0.7], [0.6,0.4], [0.4, 0.6],
                      [0.5, 0.5], [0.2, 0.8], [0.7, 0.3], [0.1, 0.9], [0.0, 1.0], [1.0, 0.0]]
    # train_weight_2 = [[0.0, 1.0], [1.0, 0.0], [0.9, 0.1], [0.3, 0.7], [0.5, 0.5], [0.7, 0.3], [0.1, 0.9]]
    # train_weight_2 = [[0.9,0.1]]
    # train_weight_3 = [[0.7, 0.2, 0.1]]

    # train_weight_3 = [[0.7, 0.2, 0.1], [0.2, 0.7, 0.1], [0.2, 0.1, 0.7],
    #                   [0.3, 0.3, 0.4]]
    train_weight_3 = []
    ...
    tau_alpha = [1]
    outdir_root = 'exp_consistent_repeat_no_search_epoch_sche_multistep_lr_label_splitter_more_metric_v1_order_bug_iid'
    ...
    yaml_root_dir = 'scripts/marketplace/example_scripts/ls_run_scripts_repeated_no_search_epoch_sche_multistep_lr_label_splitter_more_metric_v1_order_bug_iid'
    sh_pth = 'scripts/marketplace/rep_sh_epoch_sche_multistep_lr_label_splitter_more_metric_v1_order_bug_iid'
    change_splitter = True

    # splitter_name = 'alpha_tune_cifar10_splitter'
    # splitter_args_alpha = 0.01
    splitter_name = 'lda'
    splitter_args_alpha= 100
    add_eval_metric = True
    # metric_list = ['acc', 'correct', 'f1', 'roc_auc']
    metric_list = ['acc', 'correct', 'f1', 'roc_auc', 'classification_report', 'confusion_matrix']
        ...
        for client_bid in client_bid_2:
            for seed_val in seed:
                for train_weight_val in train_weight_2:
                    ...
                    if update_hp == False:
                        if 'update_hp' not in original_yaml['marketplace'][
                                'alpha_tune'].keys():
                            original_yaml['marketplace']['alpha_tune'][
                                'update_hp'] = update_hp
                        original_yaml['train'][
                    ...
                    for entropy_determined_val in entropy_determined:
                        ...
                        if entropy_determined_val:
                            ...
                            for tau_alpha_val in tau_alpha:
                                ...
                                yaml_sav_pth = os.path.join(
                                    yaml_root, yaml_file_name)
                                yaml_file_pth_list_two_clients.append(
                                    yaml_sav_pth)
                                if tau_alpha_val == 1:
                                    top_run_set.append(yaml_sav_pth)
                                else:
            ...
            for client_bid in client_bid_3:
                for seed_val in seed:
                    for train_weight_val in train_weight_3:
                        ...
                        original_yaml['marketplace']['alpha_tune'][
                            'client_bid'] = client_bid
                        original_yaml['federate'][
                            'total_round_num'] = total_round
                        for entropy_determined_val in entropy_determined:
                            original_yaml['marketplace']['alpha_tune'][
                                'entropy_determined'] = entropy_determined_val
                            client_bid_text = ':'.join('{}'.format(item)
                            ...
                            if entropy_determined_val:
                                ...
                                for tau_alpha_val in tau_alpha:
                                    file_post = '{}_client_{}_seed_{}_train_weight_{}_tmp_alpha_{}'.format(
                                        len(client_bid), client_bid_text,
                                        seed_val, train_weight_txt,
                                        tau_alpha_val)
                                    original_yaml['marketplace']['alpha_tune'][
                                        'tau_alpha'] = tau_alpha_val
                                    original_yaml['outdir'] = os.path.join(
    ...
    while len(optional_run_set) > 0 or len(top_run_set) > 0:
        ...
        for gpu_id in available_gpu:
            for i in range(per_gpu_running):
                ...
                count += 1
                if count == limit or (len(top_run_set) == 0
                                      and len(optional_run_set) == 0):

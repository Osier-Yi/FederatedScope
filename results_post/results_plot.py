...
def plot_alpha_tune_result(file_pth,
                           client_num,
                           plot_sav_dir,
    ...
    for plot_key in train_valid_metric_name:
        ...
        plt.savefig(
            os.path.join(plot_sav_dir,
                         file_sav_prefix + '_' + plot_key + '.pdf'))
        plt.close()
...
def plot_alpha(alpha_pth, num_client, plot_sav_dir, file_sav_prefix=None):
    ...
    for client_id in client_id_list:
        if MAXROUND > 0:
            plt.plot(alpha_df[alpha_df['round'] <= MAXROUND]['round'],
                     alpha_df[alpha_df['round'] <= MAXROUND][
    ...
    plt.savefig(
        os.path.join(plot_sav_dir,
                     file_sav_prefix + '_' + 'Alpha_value' + '.pdf'))
    plt.close()
    ...
    for dir_name in os.listdir(root_dir):
        if len(dir_name) >= len(
            ...
            if method_name not in filter_method_list:
                ...
                file_sav_prefix = '_'.join('{}'.format(item)
                                           for item in dir_mapping[key_name])
                ...
                for dirs in os.listdir(file_pth):
                    ...
                    for file in os.listdir(file_pth):
                        if file.split('.')[0] == 'eval_results' and (
                            ...
                            check_dir(plot_sav_dir)
                            plot_alpha_tune_result(final_pth, client_num,
                                                   plot_sav_dir,
                                                   file_sav_prefix)
...
if __name__ == '__main__':
    ...
    dir_mapping = {
        ...
        'results_exp_ls_epoch_alpha_tune_2_clients_0_1000_consistent_label': [
            0, 1000
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_0_100_consistent_label': [
            0, 100
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_0_10_consistent_label': [
            0, 10
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_0_50_consistent_label': [
            0, 50
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_100_0_consistent_label': [
            100, 0
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_10_0_consistent_label': [
            10, 0
        ],
        'results_exp_ls_epoch_alpha_tune_2_clients_50_0_consistent_label': [
            50, 0
        ]
    }

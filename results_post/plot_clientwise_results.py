...
train_valid_metric_name = [
    'train_acc', 'train_avg_loss', 'val_avg_loss_before', 'val_avg_loss_after'
]
...
if __name__ == '__main__':
    ...
    method_results_pth = {
        'alpha': 'Alpha_tune_results/results_3_clients_alpha',
    ...
    fedex_split_name_set = [
        'train_valid_metric', 'valid_metric', 'test_valid_metric'
    ]

    fedex_train_valid_metric_name = [
        'train_acc', 'train_avg_loss', 'val_avg_loss_before',
        'val_avg_loss_after'
    ]
    ...
    fedex_key_list = [
        fedex_train_valid_metric_name, fedex_valid_metric_name,
        fedex_test_metric_name
    ]
    ...
    for idx in range(len(split_set_key_list)):
        ...
        for plot_key in key_list:
            for client_id in range(1, client_num + 1):
                for method_name in method_name_set:
                    print('work on {}_{}_{}'.format(plot_key, client_id,
                                                    method_name))
                    if method_name == 'plain_fedex':
                    ...
                    if MAXROUND > 0:
                        plt.plot(data[data['round'] <= MAXROUND]['round'],
                                 data[data['round'] <= MAXROUND][plot_key],
                                 label='method: {}'.format(method_name))
                    else:
                        plt.plot(data['round'],
                                 data[plot_key],
                                 label='method: {}'.format(method_name))
                ...
                plt.savefig(
                    os.path.join(
                        plot_sav_dir,
                        '{}_{}_{}'.format(plot_key, plot_key, client_id)))
                plt.close()

...
def table_data_collect(dir_pth, dir_mapping=None):
    ...
    for dir_name in os.listdir(dir_pth):
        print(dir_name)
        if dir_name[0]!='.':
            if 'valid_metric' not in os.listdir(os.path.join(dir_pth, dir_name)):
                if dir_mapping is not None:
                    pass
                else:
                    table_txt += '{} & '.format(dir_name)
                tmp = pd.read_csv(os.path.join(dir_pth, dir_name, 'test_valid_metric'))
                tmp_train = pd.read_csv(os.path.join(dir_pth, dir_name, 'train_valid_metric'))
                alpha_pd = pd.read_csv(os.path.join(dir_pth, dir_name, 'alpha'))
                # data = {}
                ...
                for client_id in range(1, client_num+1):
                    ...
                    data.append(alpha_pd[(alpha_pd['round']==Round)]['client_{}'.format(client_id)].iloc[0])
                    data.append(tmp[(tmp['round']==Round) & (tmp['client']==client_id)]['test_acc'].iloc[0])
                    data.append(tmp['val_acc'][(tmp['round'] == Round) & (tmp['client']==client_id)].iloc[0])
                    data.append(tmp_train['train_acc'][(tmp_train['round'] == Round) & (tmp_train['client']==client_id)].iloc[0])
                    # print(data)
                    if dir_mapping is not None:
                        table_txt += '{} & {} & ' .format(client_id, dir_mapping[dir_name][client_id-1])
                        table_txt += ' & '.join('{:.4f}'.format(item) for item in data)
                    else:
                        table_txt += ' & '.join('{:.4f}'.format(item) for item in data)
                    table_txt += '\\\ \n'
